...
enum : uint128_t {
    ...
    // ctrl0, bit 7 -- Clear Carry Flag
    CLC                                 = U128_ENUM(0b1ul, 7, 0),


    // ctrl0, bit 6 -- Set Carry Flag
    STC                                 = U128_ENUM(0b1ul, 6, 0),


    // ctrl0, bit 5 -- Latch Z Flag (Pgm)
    PGM_Z_LATCH                         = U128_ENUM(0b1ul, 5, 0),


    // ctrl0, bit 4 -- Latch Z Flag (Interrupt)
    INT_Z_LATCH                         = U128_ENUM(0b1ul, 4, 0),


    // ctrl0, bit 3 -- Latch C Flag (Pgm)
    PGM_C_LATCH                         = U128_ENUM(0b1ul, 3, 0),


    // ctrl0, bit 2 -- Latch C Flag (Interrupt)
    INT_C_LATCH                         = U128_ENUM(0b1ul, 2, 0),


    // ctrl0, bits 1:0 -- Address Bus 1 Assert
    ...
    // ctrl1, bit 7 -- Latch N Flag (Pgm)
    PGM_N_LATCH                         = U128_ENUM(0b1ul, 7, 1),


    // ctrl1, bit 6 -- Latch N Flag (Interrupt)
    INT_N_LATCH                         = U128_ENUM(0b1ul, 6, 1),


    // ctrl1, bit 5 -- Latch V Flag (Pgm)
    PGM_V_LATCH                         = U128_ENUM(0b1ul, 5, 1),


    // ctrl1, bit 4 -- Latch V Flag (Interrupt)
    INT_V_LATCH                         = U128_ENUM(0b1ul, 4, 1),


    // ctrl1, bits 3:0 -- Address Bus 2 Assert
    ...
    // ctrl2, bit 7 -- Latch L Flag (Pgm)
    PGM_L_LATCH                         = U128_ENUM(0b1ul, 7, 2),


    // ctrl2, bit 6 -- Latch L Flag (Interrupt)
    INT_L_LATCH                         = U128_ENUM(0b1ul, 6, 2),


    // ctrl2, bit 5 -- Clear oVerflow Flag
    CLV                                 = U128_ENUM(0b1ul, 5, 0),


    // ctrl2, bit 4 -- Set oVerflow Flag
    STV                                 = U128_ENUM(0b1ul, 4, 0),


    // ctrl2, bits 3:0 -- ALU A Assert
    ...
    // ctrl3, bits 7:4 -- ALU Logic Unit Results
    ...
    // ctrl4, bit 7 -- Instruction Suppress
    INSTRUCTION_SUPPRESS                = U128_ENUM(0b1ul, 7, 4),


    // ctrl4, bits 6:0 -- assert to Main Bus
    ...
    // ctrl5, bits 7:2 -- unused


    // ctrl5, bits 1:0 -- Carry Select
    ...
    // ctrl6, bits 7:6 -- unused

    // ctrl6, bit 5 -- Break
    BREAK                               = U128_ENUM(0b1ul, 5, 6),

    // ctrl6, bit 4 -- Leave interrupt Context
    LEAVE_INT_CONTEXT                   = U128_ENUM(0b1ul, 4, 6),

    // ctrl6, bit 3 -- fetch suppress
    FETCH_SUPPRESS                      = U128_ENUM(0b1ul, 3, 6),

    // ctrl6, bits 2:0 -- shift and rotate signals
...
uint128_t GenerateControlSignals(int loc)
{
    ...
    case OPCODE_ADD_R1_IMM:
        ...
        return out | CARRY_SELECT_0 | ALU_BUS_A_ASSERT_R1 | ALU_BUS_B_ASSERT_FETCH | MAIN_BUS_ASSERT_ALU_RESULT |
                R1_LOAD | PGM_Z_LATCH | PGM_C_LATCH | PGM_N_LATCH | PGM_V_LATCH | PGM_L_LATCH | INSTRUCTION_SUPPRESS;
    ...
    case OPCODE_ADD_R2_IMM:
        ...
        return out | CARRY_SELECT_0 | ALU_BUS_A_ASSERT_R2 | ALU_BUS_B_ASSERT_FETCH | MAIN_BUS_ASSERT_ALU_RESULT |
                R2_LOAD | PGM_Z_LATCH | PGM_C_LATCH | PGM_N_LATCH | PGM_V_LATCH | PGM_L_LATCH | INSTRUCTION_SUPPRESS;
