...
void Conditionals::AddOpcodes(const std::string opcode, const std::string bytes)
{
    ...
    OpCodes::Add(strdup(opcode.c_str()), strdup((std::string(v) + bytes).c_str()));

    for (auto it = map.begin(); it != map.end(); ++ it) {
        sprintf(v, "%lX", it->second->Value());
        OpCodes::Add(strdup(it->second->GenerateOpcode(opcode).c_str()), strdup((std::string(v) + bytes).c_str()));
    }
}
