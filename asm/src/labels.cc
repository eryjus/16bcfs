...
void FixupLocation(struct Label_t *lbl)
{
    ...
    while (fix) {
        Binary_t::SetAddressAt(fix->where, lbl->location);
...
void NoteLabelLocation(void)
{
    ...
    for (int i = 0; i < MAX_LABELS; i ++) {
        if (!labels[i].name) {
            ...
            labels[i].location = Binary_t::GetLocation();
            labels[i].fixes = 0;
        ...
        if (strcmp(labels[i].name, yylval.name) == 0) {
            ...
            Messaging::Error("Duplicate Label: %s", Parser_t::GetSourceFile(), yylineno, 0, 0, yylval.name);
            return;
        }
    }
}
...
void DumpLabelLocations(void)
{
    for (int i = 0; i < MAX_LABELS; i ++) {
        ...
        Messaging::Msg("Label `%s` is refers to address %x", labels[i].name, labels[i].location);

        if (labels[i].fixes) Messaging::Error("Undefined labels %s", 0, 0, 0, 0, labels[i].name);
    }
}
